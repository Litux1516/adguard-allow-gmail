...
class DomainBlocklistConverter:
    ...
    def categories(self):
        ...
        def write_file(path, category, entries, line_prefix="", line_postfix=""):
            """
            ...
            with open(path, "w") as f:
                ...
                for entry in entries:
                    if entry != "":
                        f.write(f"{line_prefix}{entry}{line_postfix}\n")
        ...
        for category, entries in self.data.items():
            ...
            filename = "adg-blk_" + category.replace(" ", "").lower()
            filepath = Path(self.CATEGORIES_PATH).joinpath(filename)
            text_file = filepath.with_suffix(".txt")
            parsed_file = str(filepath) + "parsed"
            write_file(text_file, category, entries, line_prefix="||", line_postfix="^")
            
            filename = "adg-alw_" + category.replace(" ", "").lower()
            filepath = Path(self.CATEGORIES_PATH).joinpath(filename)
            text_file = filepath.with_suffix(".txt")
            parsed_file = str(filepath) + "parsed"
            write_file(text_file, category, entries, line_prefix="@@||", line_postfix="^")
           
            #write_file(text_file, category, entries, line_prefix="@@|| ")
